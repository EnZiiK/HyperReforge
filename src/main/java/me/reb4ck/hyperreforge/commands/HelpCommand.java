package me.reb4ck.hyperreforge.commands;
...
public class HelpCommand extends Command {
    ...
    public void execute(CommandSender cs, String[] args) {
        ...
        int current = 0;
        p.sendMessage(me.reb4ck.hyperreforge.utils.StringUtils.color(HyperReforge.getInstance().getMessages().getMessage("helpHeader")));
        for (Command command : HyperReforge.getInstance().getCommandManager().commands) {
            if ((p.hasPermission(command.getPermission()) || command.getPermission().equalsIgnoreCase("") || command.getPermission().equalsIgnoreCase("hyperreforge.")) && command.isEnabled()) {
                if (current >= (page - 1) * 18 && current < page * 18)
                    p.sendMessage(me.reb4ck.hyperreforge.utils.StringUtils.color(HyperReforge.getInstance().getMessages().getMessage("helpMessage").replace("%command%", command.getUsage()).replace("%description%", command.getDescription())));
                current++;
            }
        }
        BaseComponent[] components = TextComponent.fromLegacyText(me.reb4ck.hyperreforge.utils.StringUtils.color(HyperReforge.getInstance().getMessages().getMessage("helpfooter").replace("%maxpage%", maxpage + "").replace("%page%", page + "")));
